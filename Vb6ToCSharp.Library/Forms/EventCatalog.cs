...
public enum ArgRole
{
    Other, KeyAscii, KeyCode, KeyShift, MouseShift, Button, X, Y, Cancel, CancelEdit, UnloadMode,
    Node, Item, Column, ToolButton, Panel, PreviousTab, ItemIndex, Date, NewString, PropBag,
}
...
public sealed class EventBinding
{
    ...
    public ArgRole[] Roles { get; set; } = new ArgRole[0];
...
public static class EventCatalog
{
    ...
    private static EventBinding B(string vb, string net, string del = "System.EventHandler", string args = "System.EventArgs", string guard = "", params ArgRole[] roles) =>
        new() { VbEvent = vb, NetEvent = net, Delegate = del, Args = args, Guard = guard, Roles = roles };

    private static EventBinding Special(string vb, string what, params ArgRole[] roles) => new() { VbEvent = vb, Special = what, Roles = roles };

    /// <summary>Events every visual control has.</summary>
    private static EventBinding Common(string vbEvent, UiTarget ui)
    {
        var mouse = new[] { ArgRole.Button, ArgRole.MouseShift, ArgRole.X, ArgRole.Y };
        var key = new[] { ArgRole.KeyCode, ArgRole.KeyShift };
        if (ui == UiTarget.WinForms)
        {
            switch (vbEvent)
            {
                ...
                case "KeyPress": return B("KeyPress", "KeyPress", WF + "KeyPressEventHandler", WF + "KeyPressEventArgs", "", ArgRole.KeyAscii);
                ...
                case "Validate": return B("Validate", "Validating", "System.ComponentModel.CancelEventHandler", "System.ComponentModel.CancelEventArgs", "", ArgRole.Cancel);
        ...
        switch (vbEvent)
        {
            ...
            case "KeyPress": return B("KeyPress", "PreviewTextInput", WI + "TextCompositionEventHandler", WI + "TextCompositionEventArgs", "", ArgRole.KeyAscii);
    ...
    public static EventBinding Resolve(ControlInfo info, string vbEvent, UiTarget ui)
    {
        ...
        switch (cls)
        {
            ...
            case "ListBox":
                ...
                if (vbEvent == "ItemCheck" && wf) return B("ItemCheck", "ItemCheck", WF + "ItemCheckEventHandler", WF + "ItemCheckEventArgs", "", ArgRole.ItemIndex);
                break;
            case "ComboBox":
            case "ImageCombo":
                if (vbEvent == "Click") return sel;
            ...
            case "MonthView":
                ...
                if (vbEvent == "DateClick" && wf) return B("DateClick", "DateSelected", WF + "DateRangeEventHandler", WF + "DateRangeEventArgs", "", ArgRole.Date);
                break;
            case "DirListBox":
            case "DriveListBox":
            ...
            case "TreeView":
                if (wf)
                {
                    ...
                    if (vbEvent == "NodeClick") return B("NodeClick", "AfterSelect", tv, tva, "", ArgRole.Node);
                    if (vbEvent == "Expand") return B("Expand", "AfterExpand", tv, tva, "", ArgRole.Node);
                    if (vbEvent == "Collapse") return B("Collapse", "AfterCollapse", tv, tva, "", ArgRole.Node);
                    if (vbEvent == "NodeCheck") return B("NodeCheck", "AfterCheck", tv, tva, "", ArgRole.Node);
                    if (vbEvent == "AfterLabelEdit") return B("AfterLabelEdit", "AfterLabelEdit", WF + "NodeLabelEditEventHandler", WF + "NodeLabelEditEventArgs", "", ArgRole.CancelEdit, ArgRole.NewString);
                }
                else if (vbEvent == "NodeClick")
                {
                    return B("NodeClick", "SelectedItemChanged", "System.Windows.RoutedPropertyChangedEventHandler<object>", "System.Windows.RoutedPropertyChangedEventArgs<object>", "", ArgRole.Node);
                }
                break;
            case "ListView":
                if (wf)
                {
                    if (vbEvent == "ItemClick") return B("ItemClick", "ItemSelectionChanged", WF + "ListViewItemSelectionChangedEventHandler", WF + "ListViewItemSelectionChangedEventArgs", "if (!e.IsSelected) return;", ArgRole.Item);
                    if (vbEvent == "ColumnClick") return B("ColumnClick", "ColumnClick", WF + "ColumnClickEventHandler", WF + "ColumnClickEventArgs", "", ArgRole.Column);
                    if (vbEvent == "ItemCheck") return B("ItemCheck", "ItemChecked", WF + "ItemCheckedEventHandler", WF + "ItemCheckedEventArgs", "", ArgRole.Item);
                }
                else if (vbEvent == "ItemClick")
                {
                    return B("ItemClick", "SelectionChanged", WC + "SelectionChangedEventHandler", WC + "SelectionChangedEventArgs", "", ArgRole.Item);
                }
                break;
            case "Toolbar":
                if (vbEvent == "ButtonClick")
                {
                    return wf ? B("ButtonClick", "ItemClicked", WF + "ToolStripItemClickedEventHandler", WF + "ToolStripItemClickedEventArgs", "", ArgRole.ToolButton)
                              : B("ButtonClick", "ButtonBase.Click", "System.Windows.RoutedEventHandler", "System.Windows.RoutedEventArgs", "", ArgRole.ToolButton);
                }
                break;
            case "StatusBar":
                if (vbEvent == "PanelClick" && wf) return B("PanelClick", "ItemClicked", WF + "ToolStripItemClickedEventHandler", WF + "ToolStripItemClickedEventArgs", "", ArgRole.Panel);
                break;
            case "TabStrip":
            case "SSTab":
                if (vbEvent == "Click")
                {
                    var roles = cls == "SSTab" ? new[] { ArgRole.PreviousTab } : new ArgRole[0];
                ...
                if (vbEvent == "BeforeClick" && wf) return B("BeforeClick", "Selecting", WF + "TabControlCancelEventHandler", WF + "TabControlCancelEventArgs", "", ArgRole.Cancel);
                break;
            case "MSFlexGrid":
            case "MSHFlexGrid":
    ...
    private static EventBinding RootEvent(string vbType, string vbEvent, UiTarget ui)
    {
        ...
        if (vbType == "VB.UserControl" || vbType == "VB.PropertyPage")
        {
            switch (vbEvent)
            {
                ...
                case "ReadProperties": return Special("ReadProperties", "ReadProperties", ArgRole.PropBag);
                case "WriteProperties": return Special("WriteProperties", "WriteProperties", ArgRole.PropBag);
        ...
        switch (vbEvent)
        {
            ...
            case "Unload":
                return wf ? B("Unload", "FormClosing", WF + "FormClosingEventHandler", WF + "FormClosingEventArgs", "", ArgRole.Cancel)
                          : B("Unload", "Closing", "System.ComponentModel.CancelEventHandler", "System.ComponentModel.CancelEventArgs", "", ArgRole.Cancel);
            case "QueryUnload":
                return wf ? B("QueryUnload", "FormClosing", WF + "FormClosingEventHandler", WF + "FormClosingEventArgs", "", ArgRole.Cancel, ArgRole.UnloadMode)
                          : B("QueryUnload", "Closing", "System.ComponentModel.CancelEventHandler", "System.ComponentModel.CancelEventArgs", "", ArgRole.Cancel, ArgRole.UnloadMode);
