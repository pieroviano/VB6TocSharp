...
public sealed class WpfEmitter
{
    ...
    private string ResourceFolder => file.IsUserControl ? "UserControls" : "Forms";

    private EmitResult Run()
    {
        var root = file.Root;
        var hosted = file.AllControls().Any(c => ctx.Info(c).Kind == ControlKind.Hosted);
        ...
        if (!uc)
        {
            ...
            a.Add("Background=" + Xml(root.Has("BackColor") ? VbColor.Xaml(VbColor.Parse(root.Get("BackColor").Raw)) : VbColor.Xaml(0x8000000F)));
            if (file.IsMdiForm) xaml.Append("<!-- TODO: WPF has no MDI; ").Append(ctx.ClassName).Append(" was an MDIForm (child forms open as separate windows) -->\r\n");
        }
        else
        {
            a.Add("Width=" + Xml(w.ToString(Inv)));
            a.Add("Height=" + Xml(h.ToString(Inv)));
            if (root.Has("BackColor")) a.Add("Background=" + Xml(VbColor.Xaml(VbColor.Parse(root.Get("BackColor").Raw))));
        }
        if (root.Has("ForeColor")) a.Add("Foreground=" + Xml(VbColor.Xaml(VbColor.Parse(root.Get("ForeColor").Raw))));
    ...
    private string Resource(VbControl c, string vbProp, string suffix, bool icon = false)
    {
        var res = Picture(file, c, vbProp, "");
        ...
        var fname = ctx.ClassName + "." + (c == file.Root ? "Form" : c.MemberName) + "." + suffix + FrxReader.Extension(res.Kind);
        res.Name = ResourceFolder + "\\Resources\\" + fname;
        result.Resources.Add(res);
        return "Resources/" + fname;
    }

    private IEnumerable<string> FontAttributes(VbControl c)
    {
        string name;
        double size;
        bool bold, italic;
    ...
    private bool IsCodeWired(VbControl c, EventBinding b) =>
        (c != file.Root && ctx.Arrays.Contains(c.Name)) || ctx.Info(c).Kind is ControlKind.NonVisual or ControlKind.Hosted || b.Direct;

    private IEnumerable<string> EventAttributes(VbControl c)
    {
        var keyPreview = c == file.Root && c.Bool("KeyPreview");
        foreach (var (ev, b, handler) in ctx.EventsOf(c))
        {
            if (c == file.Root && codeWiredLoad && ev == "Load") continue;
            if (IsCodeWired(c, b))
            {
    ...
    private void CodeWire(VbControl c, EventBinding b, string handler)
    {
        var info = ctx.Info(c);
        if (b.Direct)
        {
        ...
        if (ctx.Arrays.Contains(c.Name) && c != file.Root)
        {
            if (c.Index != file.AllControls().Where(x => x.Name == c.Name).Min(x => x.Index)) return;
    ...
    private void RegisterArray(VbControl c, string clrType)
    {
    ...
    private string Brush(VbControl c, string prop) => VbColor.Xaml(VbColor.Parse(c.Get(prop).Raw));

    private List<string> Placement(VbControl c, bool canvas)
    {
    ...
    private void Common(VbControl c, List<string> a)
    {
        ...
        if (c.Has("ToolTipText")) a.Add("ToolTip=" + Xml(TextOf(file, c, "ToolTipText")));
    ...
    private void Children(VbControl container, bool canvas)
    {
        foreach (var c in container.Children) Control(c, canvas);
    }

    private void Control(VbControl c, bool canvas)
    {
        var info = ctx.Info(c);
        ...
        {
            case "Label":
                a.Add("Content=" + Xml(AccessText(TextOf(file, c, "Caption") ?? "")));
                a.Add("Padding=\"0\"");
            ...
            case "OptionButton":
                ...
                break;
            case "Frame":
                a.Add("Header=" + Xml(AccessText(TextOf(file, c, "Caption") ?? "")));
                if (c.Num("BorderStyle", 1) == 0) a.Add("BorderThickness=\"0\"");
            ...
            case "ImageCombo":
                ...
                if (c.Num("Style") != 2 && c.Has("Text")) a.Add("Text=" + Xml(TextOf(file, c, "Text")));
                if (c.Bool("Sorted")) content.Add("<!-- TODO: Sorted: sort Items.SortDescriptions -->");
                foreach (var it in ListOf(file, c, "List")) content.Add("<ComboBoxItem Content=" + Xml(it) + " />");
            ...
            case "ListBox":
                ...
                foreach (var it in ListOf(file, c, "List")) content.Add("<ListBoxItem Content=" + Xml(it) + " />");
                if (c.Has("ItemData")) content.Add("<!-- TODO: ItemData values not converted (store them in Tag) -->");
                break;
            case "PictureBox":
            ...
            case "RichTextBox":
                ...
                {
                    content.Add("<FlowDocument><Paragraph><Run Text=" + Xml(TextOf(file, c, "Text")) + " /></Paragraph></FlowDocument>");
                }
                if (c.Bool("Locked")) a.Add("IsReadOnly=\"True\"");
                break;
        }
    ...
    private void TabItems(VbControl c)
    {
    ...
    private string Stroke(VbControl c)
    {
    ...
    private void LineShape(VbControl c)
    {
    ...
    private void ShapeElement(VbControl c)
    {
    ...
    private void NonVisual(VbControl c, ControlInfo info)
    {
        var me = c.MemberName;
        var type = info.Wpf;
    ...
    private void Hosted(VbControl c, ControlInfo info, bool canvas)
    {
    ...
    private bool Handlerless(VbControl c, string ev) =>
        System.Text.RegularExpressions.Regex.IsMatch(file.Code, @"Sub\s+" + System.Text.RegularExpressions.Regex.Escape(ctx.HandlerName(c, ev)) + @"\s*\(\s*\)", System.Text.RegularExpressions.RegexOptions.IgnoreCase);

    private void UserControlProperties(VbControl c)
    {
