...
public static class Units
{
    ...
    public static VbControl ScaleOwner(VbControl c)
    {
    ...
    public static bool HasScale(VbControl c) => c.Type is "VB.Form" or "VB.MDIForm" or "VB.UserControl" or "VB.PictureBox" or "VB.PropertyPage";

    /// <summary>Twips per scale unit on the X / Y axis of <paramref name="owner"/>.</summary>
    public static double Factor(VbControl owner, bool y)
    {
    ...
    public static double ClientTwips(VbControl owner, bool y)
    {
    ...
    public static double PosTwips(VbControl c, string prop)
    {
        var y = IsY(prop);
    ...
    public static double SizeTwips(VbControl c, string prop, double def = 0)
    {
        if (!c.Has(prop)) return def;
        return c.Num(prop) * Factor(ScaleOwner(c), IsY(prop));
    }

    public static int PosPx(VbControl c, string prop) => ToPixels(PosTwips(c, prop));
    public static int SizePx(VbControl c, string prop, double defTwips = 0) => ToPixels(SizeTwips(c, prop, defTwips));
