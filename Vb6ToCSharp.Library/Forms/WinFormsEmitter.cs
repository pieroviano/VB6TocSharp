...
public sealed class WinFormsEmitter
{
    ...
    private readonly VbFormFile file;
    ...
    private string Ref(VbControl c) => c == file.Root ? "this" : "this." + c.MemberName;

    /// <summary>Concrete .NET type, refined by properties (borderless Frame → Panel, checked ListBox, oval Shape…).</summary>
    public static string EffectiveType(VbControl c, ControlInfo info)
    {
        var t = info.WinForms;
        switch (c.Type)
        {
    ...
    private EmitResult Run(string ns)
    {
        ...
        toolTip = file.AllControls().Any(c => c.Has("ToolTipText") && c.Type != "VB.Menu");
    ...
    private void Declare(VbControl c, string type)
    {
    ...
    private void Visit(VbControl c)
    {
        var info = ctx.Info(c);
    ...
    private void AddChildren(VbControl container, string me)
    {
    ...
    private void Shapes(VbControl container, string me)
    {
    ...
    private void Wire(VbControl c)
    {
        ...
        foreach (var (ev, b, handler) in ctx.EventsOf(c))
        {
            ...
            if (c.IsArrayElement)
            {
                if (c.Index != file.AllControls().Where(x => x.Name == c.Name).Min(x => x.Index)) continue; // once per array
    ...
    private bool EventHasArgs(VbControl c, string ev)
    {
    ...
    private void Image(VbControl c, string me, string vbProp, string netProp, bool icon = false)
    {
        var name = (c == file.Root ? "$this" : c.MemberName) + "." + netProp;
        var res = Picture(file, c, vbProp, name);
        if (res == null) return;
    ...
    private void RootProperties(VbControl root, ControlInfo info)
    {
        const string me = "this";
        Section(ctx.ClassName);
        var uc = file.IsUserControl || root.Type == "VB.PropertyPage";
        ...
        if (root.Has("BackColor")) Add(me, "BackColor", VbColor.WinForms(VbColor.Parse(root.Get("BackColor").Raw)));
        if (root.Has("ForeColor")) Add(me, "ForeColor", VbColor.WinForms(VbColor.Parse(root.Get("ForeColor").Raw)));
        ...
        if (!uc)
        {
            ...
            if (file.IsMdiForm) Add(me, "IsMdiContainer", "true");
            Image(root, me, "Icon", "Icon", true);
            Image(root, me, "Picture", "BackgroundImage");
            foreach (var c in file.AllControls().Where(x => x.Type == "VB.CommandButton"))
            {
    ...
    private void FontProperty(VbControl c, string me)
    {
        string name;
        double size;
    ...
    private void CursorProperty(VbControl c, string me)
    {
    ...
    private (int x, int y) Location(VbControl c)
    {
    ...
    private void CommonProperties(VbControl c, ControlInfo info, string type, string me)
    {
        var (x, y) = Location(c);
        ...
        if (c.Has("BackColor")) Add(me, "BackColor", VbColor.WinForms(VbColor.Parse(c.Get("BackColor").Raw)));
        if (c.Has("ForeColor")) Add(me, "ForeColor", VbColor.WinForms(VbColor.Parse(c.Get("ForeColor").Raw)));
        if (c.Has("RightToLeft") && c.Bool("RightToLeft")) Add(me, "RightToLeft", W + "RightToLeft.Yes");
        if (c.Has("Tag")) Add(me, "Tag", Lit(c.Text("Tag")));
        FontProperty(c, me);
        CursorProperty(c, me);
        if (toolTip && c.Has("ToolTipText")) body.Add("this.ToolTipMain.SetToolTip(" + me + ", " + Lit(TextOf(file, c, "ToolTipText")) + ");");
    ...
    private void Caption(VbControl c, string me, string prop = "Text")
    {
        var t = TextOf(file, c, "Caption");
        if (t != null) Add(me, prop, Lit(t));
    }

    private void Items(VbControl c, string me)
    {
        var list = ListOf(file, c, "List");
        if (list.Count > 0) body.Add(me + ".Items.AddRange(new object[] { " + string.Join(", ", list.Select(Lit)) + " });");
        var data = ItemDataOf(file, c, "ItemData");
    ...
    private void UseVisualStyle(VbControl c, string me) => Add(me, "UseVisualStyleBackColor", B(!c.Has("BackColor")));

    private void ScrollRange(VbControl c, string me, bool vbMax = true)
    {
    ...
    private void TypeProperties(VbControl c, ControlInfo info, string type, string me)
    {
        switch (c.Type)
        {
            case "VB.Label":
            ...
            case "VB.ComboBox":
                ...
                Items(c, me);
                if (st != 2 && c.Has("Text")) Add(me, "Text", Lit(TextOf(file, c, "Text")));
                return;
            case "VB.ListBox":
    ...
    private void ImageListRef(VbControl c, string me, string vbProp, string netProp)
    {
        var il = c.Text(vbProp);
        if (il == "") return;
        var target = file.Find(il);
        if (target == null)
        {
    ...
    private void StatusPanels(VbControl c, string me)
    {
    ...
    private void ToolbarButtons(VbControl c, string me)
    {
        ImageListRef(c, me, "ImageList", "ImageList");
        var i = 0;
    ...
    private void TabPages(VbControl c, string me)
    {
        var tabs = (int)c.Num("Tabs", 3);
        var owner = new Dictionary<VbControl, int>();
    ...
    private void ShapeProperties(VbControl c, string me)
    {
        Add(me, "Name", Lit(c.MemberName));
        if (c.Has("Visible") && !c.Bool("Visible", true)) Add(me, "Visible", "false");
        if (c.Has("BorderColor")) Add(me, "BorderColor", VbColor.WinForms(VbColor.Parse(c.Get("BorderColor").Raw)));
        ...
        if (fs != 1)
        {
            ...
            Add(me, "FillColor", c.Has("FillColor") ? VbColor.WinForms(VbColor.Parse(c.Get("FillColor").Raw)) : D + "Color.Black");
        }
    ...
    private void NonVisualProperties(VbControl c, string me)
    {
        switch (c.ClassName)
        {
            case "Timer":
            ...
            case "ImageList":
                ...
                if (c.Has("MaskColor") && c.Bool("UseMaskColor", true)) Add(me, "TransparentColor", VbColor.WinForms(VbColor.Parse(c.Get("MaskColor").Raw)));
                var i = 0;
    ...
    private void OcxProperties(VbControl c, string me)
    {
    ...
    private void Menus(VbControl root)
    {
    ...
    private void MenuItem(VbControl m)
    {
