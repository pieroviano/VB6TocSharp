...
using static Vb6ToCSharp.CodeGeneration.UsingEverything;
...
public static class CodeConverter
{
    ...
    public static bool ConvertForm(string frmFile, bool uiOnly = false)
    {
        ...
        var s = PragmaConverter.PreProcess(ReadEntireFile(frmFile));
        ...
        try
        {
            ...
            var ppHeader = StatementsConverter.BeginFile(s, ProjectInfo().CondComp); // #Const -> #define
            ...
            convertForm = WriteOut(f, PragmaConverter.PostProcess(x), frmFile); // was never set: always False
            return convertForm;
        }
        finally
        {
    ...
    {
        if (projectInfo == null || !string.Equals(projectInfo.Path, VbpFile, StringComparison.OrdinalIgnoreCase)) projectInfo = Parsing.Model.ProjectInfo.Load(VbpFile);
        return projectInfo;
    }
    ...
    public static bool ConvertModule(string basFile)
    {
        ...
        var s = PragmaConverter.PreProcess(ReadEntireFile(basFile));
        var fName = ModuleName(s);
        currentModule = fName;
        var f = fName + ".cs";
        ...
        convertModule = WriteOut(f, PragmaConverter.PostProcess(x), basFile); // was never set: always False
        return convertModule;
    }
    ...
    public static bool ConvertClass(string clsFile)
    {
        ...
        var s = PragmaConverter.PreProcess(ReadEntireFile(clsFile));
        var fName = ModuleName(s);
        currentModule = fName;
        var f = fName + ".cs";
        ...
        f = fName + ".cs";
        convertClass = WriteOut(f, PragmaConverter.PostProcess(x), clsFile); // was never set: always False
        return convertClass;
    }
    ...
    public static string ConvertClassSource(string s, System.Collections.Generic.IDictionary<string, string> projectConstants = null)
    {
        var fName = ModuleName(s);
        currentModule = fName;
        StatementsConverter.FileHeader = StatementsConverter.BeginFile(s, projectConstants); // #Const -> #define
        var model = ClassDefinition.Scan(fName, s);
        ClassesConverter.Register(model);
        if (ClassesConverter.IsInterface(fName))
        {
            var iface = ClassesConverter.InterfaceDeclaration(model);
            if (iface != null)
            {
                return iface;
            }
        }
        ClassesConverter.Current = model;
        try
        {
            var code = Mid(s, CodeSectionLoc(s));
            var j = CodeSectionGlobalEndLoc(code);
            ...
            var x = "";
            x = x + ClassesConverter.ClassDeclaration(model) + vbCrLf;
            x = x + ClassesConverter.ClassMembers(model);
            x = x + globals + vbCrLf + vbCrLf + functions;
            x = x + vbCrLf + "}";
            return x;
        }
    ...
    public static string SanitizeCode(string str, bool midLine = false)
    {
        ...
        foreach (var iterL in sp)
        {
            ...
            l = DeComment(l);
            l = StatementsConverter.EscapeKeywords(DeString(l)); // VB6 names that are C# keywords get a "_"

            // line number: "10 x = 1" -> "L10:" + "x = 1"
            var lineNo = System.Text.RegularExpressions.Regex.Match(l, "^\\s*([0-9]+):?(\\s+(.*))?$");
            if (lineNo.Success && !midLine)
            {
                var lineLabel = StatementsConverter.LabelName(lineNo.Groups[1].Value) + ":";
            ...
            { // single-line If: each branch is sanitized again (it may hold ':' lists or another single-line If)
                ...
                r = r + CodeConverter.SanitizeCode(SingleLineIfBranch(thenPart), true);
                if (elsePart != null)
                {
                    r = r + n + "Else";
                    r = r + CodeConverter.SanitizeCode(SingleLineIfBranch(elsePart), true);
                }
                r = r + n + "End If";
                goto NextLine;
            }
            ...
            l = System.Text.RegularExpressions.Regex.Replace(l, StatementsConverter.TimeLiteralPattern, m =>
            {
            ...
            if (NextBy(l, ":") != l)
            {
                ...
                else
                {
                    ...
                    if (f != l)
                    {
                        ...
                        r = r + CodeConverter.SanitizeCode(RestoreTime(l), true);
                    }
                }
            }
            else
    ...
    public static string CommentBlock(string str)
    {
        ...
        {
            if (StatementsConverter.IsDirective(l))
            {
                if (NlTrim(run) != "") s = s + "/*" + vbCrLf + Replace(NlTrim(run), "*/", "* /") + vbCrLf + "*/" + vbCrLf;
                run = "";
                s = s + StatementsConverter.ConvertDirective(l) + vbCrLf;
            }
            else
            {
                run = run + l + vbCrLf;
    ...
    public static string ConvertDeclare(string s, int ind, bool isGlobal = false, bool asModule = false)
    {
        ...
        foreach (var iterL in StatementsConverter.SplitTopLevel(s))
        {
            var l = iterL;
            l = Trim(l);
            if (l == "")
            ...
            var suffixType = Len(l) > 0 ? StatementsConverter.SuffixType(l[0]) : "";
            if (suffixType != "")
            { // Dim s$ / Dim n%(10)
                l = Trim(Mid(l, 2));
            }
            ...
            {
                isArr = true;
                var close = StatementsConverter.MatchParen(l, 0);
                var arraySpec = close < 0 ? "" : Mid(l, 2, close - 1);
                l = close < 0 ? "" : Trim(Mid(l, close + 2));
                if (Trim(arraySpec) != "")
                {
                    dims = StatementsConverter.SplitTopLevel(arraySpec);
                }
            }
            ...
            {
                pType = suffixType != "" ? suffixType : StatementsConverter.ImplicitType(pName);
            }

            pType = PragmaConverter.TypeOverride(pName) ?? pType; // pragma SetType
            ...
            {
                res = before + ClassesConverter.WithEventsProperty(mods, cType, pType, pName, ind);
            }
            else if (!isArr && asNew && isGlobal && StatementsConverter.AutoNew)
            ...
            else if (!isArr)
            {
                ...
                {
                    res = res + StatementsConverter.DefaultValue(pType);
                }
                res = res + ";" + vbCrLf;
            }
            else
            {
                ...
                {
                    counts.Add(StatementsConverter.DimBounds(d, out var lb, out var ub));
                    lbs.Add(lb);
                    ubs.Add(ub);
                ...
                var lower = dims.Count == 0 ? StatementsConverter.OptionBase.ToString() : lbs[0];
                vb6Array = dims.Count <= 1 && lower != "0" && !StatementsConverter.ForceZeroBounds;
                if (dims.Count == 0)
                ...
                else
                {
                    ...
                    var alloc = StatementsConverter.IsPlainValueType(cType) || dims.Count > 2 ? "new " + cType + "[" + asArray + "]" : "NewArray<" + cType + ">(" + asArray + ")";
    ...
    public static string ConvertConstant(string s, bool isGlobal = true)
    {
        ...
        var suffixType = StatementsConverter.StripSuffix(ref cName);
        if (TLeft(s, 3) == "As ")
        {
            s = Trim(Mid(Trim(s), 3));
            cType = SplitWord(s, 1);
    ...
    public static string ConvertConstants(string s, bool isGlobal = true, int ind = 0)
    {
        ...
        var r = "";
        foreach (var item in StatementsConverter.SplitTopLevel(m.Groups[2].Value))
        {
            if (item != "")
            {
    ...
    public static string ConvertEnum(string s)
    {
        ...
        for (var i = 1; i < lines.Length; i++)
        {
            ...
            if (StatementsConverter.IsDirective(t))
            {
                res = res + vbCrLf + StatementsConverter.ConvertDirective(t);
                continue;
            }
            string member;
    ...
    public static string ConvertType(string s)
    {
        ...
        StatementsConverter.RegisterUdt(eName);
        StatementsConverter.RegisterUdtFields(eName, s);
        ...
        while (TLeft(s, 8) != "End Type" && s != "")
        {
            ...
            { // conditional compilation around members
                ...
                var directive = StatementsConverter.ConvertDirective(eol > 0 ? Left(s, eol - 1) : s);
            ...
            if (LMatch(s, "("))
            {
                ...
                if (!dynamicArray)
                {
                    foreach (var d in StatementsConverter.SplitTopLevel(n))
                    {
                        counts.Add(StatementsConverter.DimBounds(d, out var lb, out _)); // VB6 bounds are inclusive
                        if (lb != "0")
                        {
            ...
            {
                eType = StatementsConverter.ImplicitType(eName);
            }
            var fixedLen = "";
            if (TLMatch(s, "*"))
            { // String * n
    ...
    public static string ConvertParameter(string s, bool neverUnused = false)
    {
        ...
        pName = Replace(pName, "()", "");
        var suffixType = StatementsConverter.StripSuffix(ref pName); // ByVal s$
        if (isArrayParam)
        {
            pName = pName + "()";
        }
    ...
    public static string ConvertPrototype(string ss, ref string returnVariable, bool asModule, ref string asName)
    {
        ...
        var suffixType = StatementsConverter.StripSuffix(ref fName); // Function Name$(...)
        asName = fName;
        ...
        if (!isSub)
        {
            ...
            {
                retType = suffixType != "" ? suffixType : StatementsConverter.ImplicitType(fName);
            }
        ...
        var iface = ClassesConverter.ImplementedInterface(asName);
        if (iface != null)
    ...
    public static string ConvertElement(string s)
    {
        ...
        var radix = StatementsConverter.ConvertRadixLiteral(s);
        if (radix != null)
        {
        ...
        s = StatementsConverter.StripTypeSuffixes(s);
        ...
        { // an array element with a member: list(i).Name -> list[i].Name
            var close = StatementsConverter.MatchParen(Trim(s), element.Length - 1);
        ...
        var dateLit = StatementsConverter.ConvertDateLiterals(s);
        ...
        if (RegExTest(Trim(s), "^" + patTokenDot + "$") && StrCnt(s, ".") == 1)
        {
            ...
            if (ClassesConverter.IsMethod(SubParam(first).asType, second))
        ...
        if (IsInStr(s, ".") && ClassesConverter.IsPredeclared(SplitWord(Trim(s), 1, ".")) && ClassesConverter.Current?.Name != SplitWord(Trim(s), 1, "."))
        ...
        if (IsInStr(convertElement, ":="))
        {
            ...
            ts = ts + ": ";
            ts = ts + CodeConverter.ConvertElement(SplitWord(convertElement, 2, ":=", true, true));
            convertElement = ts;
        }
        ...
        convertElement = System.Text.RegularExpressions.Regex.Replace(convertElement, "&[HhOo][0-9A-Fa-f]+&?(?![A-Za-z0-9_])", m => StatementsConverter.ConvertRadixLiteral(m.Value) ?? m.Value);
    ...
    public static string ConvertFunctionCall(string fCall)
    {
        ...
        tb = tb + (StatementsConverter.ConversionFunction(name) ?? StatementsConverter.RuntimeFunction(name) ?? name);
    ...
    public static string ConvertValue(string s)
    {
        ...
        s = StatementsConverter.GroupNot(s);

        if (RegExTest(s, "^-[a-zA-Z0-9_]"))
        {
            convertValue = "-" + CodeConverter.ConvertValue(Mid(s, 2));
            return convertValue;
        ...
        while (true)
        {
            ...
            raws.Add(Trim(f));
            if (Left(f, 1) == "(" && Right(f, 1) == ")" && StatementsConverter.MatchParen(f, 0) == Len(f) - 1)
            {
                parts.Add("(" + CodeConverter.ConvertValue(Mid(f, 2, Len(f) - 2)) + ")");
            }
            else if (TLMatch(f, "TypeOf "))
            {
                parts.Add(CodeConverter.ConvertValue(Trim(Mid(Trim(f), 8))));
            }
            else
            {
            ...
            for (var i = 0; i < ops.Count; i++)
            {
                ...
                switch (ops[i])
                {
                    ...
                    case "Like" when pass == 1: // Option Compare decides the case sensitivity
                        folded = "LikeOperator.LikeString(" + parts[i] + ", " + parts[i + 1] + ", CompareMethod." + (StatementsConverter.OptionCompareText ? "Text" : "Binary") + ")";
                        break;
        ...
        StatementsConverter.PromoteCurrency(raws, parts);
        for (var i = 0; i < ops.Count; i++)
        {
            if (ops[i] == "/" && (i == 0 || ops[i - 1] != "/"))
            { // VB6 "/" always divides as Double; C# would divide integers as integers
                parts[i] = StatementsConverter.AsDouble(raws[i], parts[i]);
            }
        }

        StatementsConverter.FoldStringComparisons(raws, parts, ops);
        ...
        for (var i = 0; i < ops.Count; i++)
        {
            ...
            var bitwise = !ComparisonBetweenLogicals(ops, i)
                          && (IsIntegerLiteral(raws[i]) || IsIntegerLiteral(raws[i + 1]) || StatementsConverter.IsIntegralOperand(raws[i]) || StatementsConverter.IsIntegralOperand(raws[i + 1]));
            switch (ops[i])
            {
                case "\\":
                    opN = " / ";
    ...
    public static string ConvertGlobals(string str, bool asModule = false)
    {
        ...
        foreach (var iterL in s)
        {
            ...
            else if ((x = StatementsConverter.ApplyModuleOption(l)) != null)
            ...
            else if (StatementsConverter.IsDirective(l))
            {
                o = StatementsConverter.ConvertDirective(l);
            }
            else if (LMatch(l, "Implements "))
            {
                o = ClassesConverter.Current != null ? "// VB6 " + l + " (explicit interface implementation)" : "// TODO: VB6 " + l + " (only class modules implement interfaces)";
            }
    ...
    public static string ConvertCodeLine(string s)
    {
        ...
        if (T > 0)
        {
            ...
            {
                b = target.fixedLen != "" && !IsInStr(a, "(") ? "FixedLen(" + b + ", " + target.fixedLen + ")" : StatementsConverter.ImplicitConversion(target.asType, rhs, b);
            }
            convertCodeLine = convertCodeLine + b;
            if (rhs == "Nothing" && TLeft(s, 4) == "Set " && PragmaConverter.AutoDispose != "No"
                && (PragmaConverter.AutoDispose == "Force" || ClassesConverter.HasTerminate(target.asType)))
            ...
            if (setter.Success && ClassesConverter.IsParameterizedSetter(setter.Groups[2].Value) && SubParam(setter.Groups[2].Value).asArray == "")
        ...
        else
        {
            ...
            {
                convertCodeLine = StatementsConverter.ConvertDebugPrint(rest);
            }
    ...
    private static bool IsKnownName(string name)
    {
        return name == "" || name == "Me" || name == currSub || IsFuncRef(name) || IsEnumRef(name) || IsFormRef(name) || IsModuleRef(name)
               || IsControlRef(name, formName) || StatementsConverter.IsProjectGlobal(name) || IsPropertyName(name);
    }
    ...
    private static bool IsProtectable(string t)
    {
        return !IsBlockCloser(t) && !RegExTest(t, "^[a-zA-Z_][a-zA-Z_0-9]*:$") && !LMatch(t, "Dim ") && !LMatch(t, "Static ") && !LMatch(t, "Const ")
               && !LMatch(t, "On Error ") && !LMatch(t, "On Local Error ") && !LMatch(t, "Attribute ") && !StatementsConverter.IsDirective(t);
    }

    public static string ConvertSub(string str, bool asModule = false, TriState scanFirst = TriState.UseDefault)
    {
        var convertSub = "";
        ...
        switch (scanFirst)
        {
            case TriState.UseDefault:
                var oStr = str;
                CodeConverter.ConvertSub(oStr, asModule, TriState.True);
                //                          If IsInStr(Str, "StoreStockToolTipText") Then Stop
                convertSub = CodeConverter.ConvertSub(oStr, asModule, TriState.False);
                return convertSub;
            case TriState.True:
                SubBegin();
                break;
        ...
        var errs = new StatementsConverter.ErrorScope();
        ...
        var plan = StatementsConverter.ProcedurePlan.Scan(s);
        ...
        if (scanFirst == TriState.False && !StatementsConverter.OptionExplicit)
        { // no Option Explicit: names assigned without a declaration are implicit locals
            foreach (var name in UnknownAssigned())
            {
                ...
                var implicitType = StatementsConverter.ImplicitType(name);
                decls = decls + SSpace(spIndent) + ConvertDataType(implicitType) + " " + name + " = " + StatementsConverter.DefaultValue(implicitType) + "; // VB6 implicit declaration" + vbCrLf;
            }
        }
        string route = null; // label of the GoSub routine / resumable handler being collected
        var routeBody = "";
        var routeSavedInd = 0;
        var routeSavedMode = StatementsConverter.ErrorScope.Modes.None;
        ...
        void StartRoute(string label)
        {
            ...
            errs.Mode = StatementsConverter.ErrorScope.Modes.None; // a handler runs unprotected
        ...
        string LocalFunctions()
        {
            ...
            foreach (var fn in routes)
            {
                ...
                r = r + fi + (handler ? "int " + StatementsConverter.HandlerFunction(fn[0]) : "void " + fn[0]) + "() {" + vbCrLf + fn[1]
        ...
        var savedForceZero = StatementsConverter.ForceZeroBounds;
        var savedAutoNew = StatementsConverter.AutoNew;
        var outputOff = false;
        var parseOff = false;
        ...
        foreach (var iterL in s)
        {
            ...
            var pragma = PragmaConverter.Parse(l);
            if (pragma != null)
            { // VB Migration Partner pragma
                switch (pragma.Name)
                {
                    ...
                    case "Note":
                        ...
                        break;
                    default:
                        if (!PragmaConverter.ApplySetting(pragma))
                        {
            ...
            if (!isProto && !isDecl)
            {
                l = StatementsConverter.StripTypeSuffixes(l);
            }
            if (TLMatch(l, "Let "))
            {
                l = TMid(l, 5);
            ...
            if (t != "" && !isProto)
            {
                ...
                else if (errs.Mode == StatementsConverter.ErrorScope.Modes.GoTo && !errs.Resumable && !errs.TryOpen && IsProtectable(t))
                {
            ...
            else if (StatementsConverter.IsDirective(t))
            {
                ...
                o = SSpace(ind) + StatementsConverter.ConvertDirective(t);
            }
            else if (isProto)
            {
                //      CurrSub = nextBy(L, "(", 1)
            ...
            { // the handler tells the failed statement how to go on
                ...
                var code = target == "Next" ? 0 : target == "" || target == "0" ? 1 : plan.Handlers[route].ResumeLabels.IndexOf(StatementsConverter.LabelName(target)) + 2;
            ...
            else if (LMatch(t, "GoSub ") && plan.GoSubTargets.Contains(StatementsConverter.LabelName(Mid(t, 7))))
            {
                o = SSpace(ind) + StatementsConverter.LabelName(Mid(t, 7)) + "();";
            }
            ...
            { // Goto Label: kept outside the protected block, so the handler (and Resume label) can jump to it
                ...
                if (errs.Mode == StatementsConverter.ErrorScope.Modes.GoTo && labelName == errs.Handler)
                {
                    errs.Mode = StatementsConverter.ErrorScope.Modes.None; // the handler itself runs unprotected
                }
            ...
            {
                o = SSpace(ind) + "goto " + StatementsConverter.LabelName(Mid(t, 6)) + ";";
            }
            else if (LMatch(t, "On Error ") || LMatch(t, "On Local Error "))
            {
                o = StatementsConverter.ConvertOnError(t, errs, ref ind);
                errs.Resumable = errs.Mode == StatementsConverter.ErrorScope.Modes.GoTo && plan.Handlers.ContainsKey(errs.Handler);
            }
            else if (LMatch(t, "On ") && (x = StatementsConverter.ConvertOnGoTo(t, ind)) != null)
            {
                o = x;
            }
            else if (t == "Resume" || LMatch(t, "Resume "))
            {
                o = StatementsConverter.ConvertResume(t, ind);
            }
            else if (LMatch(t, "Static ") && hadPrototype || LMatch(t, "Dim ") && staticProc && hadPrototype)
            {
                ...
                foreach (var item in StatementsConverter.SplitTopLevel(rest))
                {
            ...
            { // Code sanitization prevents all single-line ifs.
                ...
                T = Mid(t, 4, Len(t) - 8);
                o = SSpace(ind) + "if (" + StatementsConverter.ConditionValue(T) + ") {";
                ind = ind + spIndent;
            }
            else if (TLeft(l, 7) == "ElseIf ")
            {
                ...
                o = SSpace(ind - spIndent) + "} else if (" + StatementsConverter.ConditionValue(T) + ") {";
            }
            else if (t == "Else")
            {
            ...
            else if (t == "Case Else" || LMatch(t, "Case "))
            {
                ...
                o = o + SSpace(ind) + (t == "Case Else" ? "default:" : StatementsConverter.ConvertCaseLabels(Mid(t, 6)));
                ind = ind + spIndent;
            }
            else if (t == "Do")
            {
            ...
            {
                o = o + SSpace(ind) + "while(" + StatementsConverter.ConditionValue(Mid(t, 10)) + ") {";
                OpenBreakable("Do");
                ind = ind + spIndent;
            }
            ...
            {
                o = o + SSpace(ind) + "while(" + StatementsConverter.ConditionValue(Mid(t, 7)) + ") {";
                OpenBreakable("While");
                ind = ind + spIndent;
            }
            else if (t == "Wend")
            ...
            {
                ind = ind - spIndent;
                o = o + SSpace(ind) + "} while(" + StatementsConverter.ConditionValue(Mid(t, 12)) + ");" + CloseBreakable(); // was negated like Loop Until
            }
            else if (LMatch(t, "Loop Until "))
            {
                ind = ind - spIndent;
                o = o + SSpace(ind) + "} while(!(" + StatementsConverter.ConditionValue(Mid(t, 12)) + "));" + CloseBreakable();
            }
            else if (t == "Loop")
            {
                ind = ind - spIndent;
            ...
            { // "Next j, i" closes two loops
                var loops = t == "Next" ? 1 : StatementsConverter.SplitTopLevel(Mid(t, 6)).Count;
            ...
            else if ((x = StatementsConverter.ConvertSimpleStatement(t, ind)) != null)
            {
                o = x;
            ...
            {
                o = StatementsConverter.ConvertReDim(t, ind);
                wrap = true;
            }
            else if (LMatch(t, "Erase "))
            {
                o = StatementsConverter.ConvertErase(t, ind);
                wrap = true;
            }
            ...
            else if ((x = StatementsConverter.ConvertMidStatement(t, ind) ?? StatementsConverter.ConvertLRSet(t, ind) ?? StatementsConverter.ConvertFileStatement(t, ind)
                          ?? StatementsConverter.ConvertGraphicsStatement(t, ind)) != null)
            {
                o = x;
                wrap = true;
            }
            ...
            if (wrap && errs.Mode == StatementsConverter.ErrorScope.Modes.ResumeNext && Trim(o) != "")
            {
                o = StatementsConverter.WrapResumeNext(o, ind, errs);
            }
            else if (wrap && errs.Mode == StatementsConverter.ErrorScope.Modes.GoTo && errs.Resumable && Trim(o) != "")
            {
                var leave = returnVariable != "" ? "return " + returnVariable + ";" : hadPrototype ? "return;" : ExitPropertyMark;
                o = StatementsConverter.WrapResumable(o, ind, errs, plan, leave, ref retries);
            }
            o = pre + o;
            ...
            o = CodeConverter.PostConvertCodeLine(o);
            o = ProjectSpecificConverter.ProjectSpecificPostCodeLineConvert(o);
        ...
        StatementsConverter.ForceZeroBounds = savedForceZero;
        StatementsConverter.AutoNew = savedAutoNew;
