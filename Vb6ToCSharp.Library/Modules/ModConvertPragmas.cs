...
/// act from that point.
/// </summary>
public static class ModConvertPragmas
{
    ...
    public sealed class Pragma
    {
        public string Target = ""; // "x" in '## x.SetType Integer
        public string Name = "";
        public string Args = "";
    }
    ...
    public static bool ApplySetting(Pragma p)
    {
        switch (p.Name)
        {
            case "ArrayBounds": // Unchanged (VB6Array for non-zero bounds), ForceZero / Shift (zero-based arrays), VB6Array
                ModConvertStatements.ForceZeroBounds = p.Args == "ForceZero" || p.Args == "Shift";
                return true;
            case "AutoNew":
                ModConvertStatements.AutoNew = !Regex.IsMatch(p.Args, "^(False|No|0)$", RegexOptions.IgnoreCase);
                return true;
            case "AutoDispose":
