...
internal static class Emit
{
    ...
    public static string TextOf(FormControlFile controlFile, ControlWithType c, string prop)
    {
        var p = c.Get(prop);
        if (p == null) return null;
        var frx = p.Frx;
        if (frx == null) return p.Text;
        var r = FrxReader.Open(controlFile.FrxPath(frx));
        return r == null ? "" : r.ReadString(frx.Offset);
    }

    public static List<string> ListOf(FormControlFile controlFile, ControlWithType c, string prop)
    {
    ...
    public static List<int> ItemDataOf(FormControlFile controlFile, ControlWithType c, string prop)
    {
        var frx = c.Get(prop)?.Frx;
        var r = frx == null ? null : FrxReader.Open(controlFile.FrxPath(frx));
    ...
    public static FormResource Picture(FormControlFile controlFile, ControlWithType c, string prop, string name)
    {
        var frx = c.Get(prop)?.Frx;
        var r = frx == null ? null : FrxReader.Open(controlFile.FrxPath(frx));
    ...
    public static string ValueLiteral(ItemProperty p)
    {
        if (p.IsQuoted) return Lit(p.Text);
        var raw = p.Raw.Trim();
        if (raw.StartsWith("&H", StringComparison.OrdinalIgnoreCase)) return N(StringConvert.ToNumber(raw));
