...
{
    private static readonly List<IOcxValueConverter> ValueConverters = new();

    /// <summary>
    /// Adds the conversions of a UI package (WPF colors and brushes). The UI packages call this from a module
    /// initializer; conversions are tried in registration order, after the UI-independent ones.
    /// </summary>
    public static void AddConverter(IOcxValueConverter converter)
    {
        if (converter == null) throw new ArgumentNullException(nameof(converter));
        lock (ValueConverters)
        {
            if (!ValueConverters.Exists(c => c.GetType() == converter.GetType())) ValueConverters.Add(converter);
        }
    }

    private static IOcxValueConverter[] Converters()
    {
        lock (ValueConverters)
        {
            if (ValueConverters.Count == 0) VbRuntime.EnsureUiPackagesLoaded();
            return ValueConverters.ToArray();
        }
    }
    ...
    internal static object ConvertTo(object value, Type type)
    {
        ...
        if (type == typeof(DrawingColor)) return Vb6Color.ToColor(ToOleColor(value));
        foreach (var converter in Converters()) // the UI package's own types (WPF Color, Brush)
        {
            if (converter.TryConvert(value, type, out var converted)) return converted;
        }
    ...
    /// <summary>A VB6 design-time value as an OLE color (<c>&amp;H00BBGGRR</c>), for the UI packages' converters.</summary>
    public static int ToOleColor(object value) => unchecked((int)ToLong(value));
